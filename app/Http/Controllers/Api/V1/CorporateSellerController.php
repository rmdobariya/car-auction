...
use App\Http\Resources\CorporateSellerResource;
...
class CorporateSellerController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        $corporate_sellers = DB::table('users')
            ...
            ->get();
        $result = CorporateSellerResource::collection($corporate_sellers);
        if (count($corporate_sellers) > 0) {
            return response()->json([
                'status' => true,
                'data' => ['corporate-seller' => $result],
            ]);
        } else {
            return response()->json([
                'status' => true,
