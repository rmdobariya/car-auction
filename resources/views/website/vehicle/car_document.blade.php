            ...
            <div class="img-thumbnail mb-3 image-container">
                ...
                <div class="btn btn-list pl-0 pb-0">
                    <a href="javascript:void(0)" data-id="{{$document->id}}" data-vehicle-id="{{$document->vehicle_id}}"
                       class="delete-car-document btn btn-danger waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="{{trans('web_string.common_delete')}}">{{trans('web_string.common_delete')}}</a>
                </div>
            </div>
        </div>
    @endforeach
